...
final class DynamicTests: XCTestCase {
    ...
    func test_bindAndFire_shouldCallsListenerImmediately() {
        let initialValue = "Initial"
        let dynamic = Dynamic(initialValue)

        var listenerCalled = false
        dynamic.bindAndFire { value in
            listenerCalled = true
    ...
    func test_bindAndFire_shouldCallsListenerOnMainThreadImmediately() {
        let initialValue = "Initial"
        let dynamic = Dynamic(initialValue)
        let expectation = expectation(description: "Listener called on main thread Immediately")

        dynamic.bindAndFire { _ in
            XCTAssertTrue(Thread.isMainThread)
            expectation.fulfill()
        }
